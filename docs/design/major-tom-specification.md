...
A location Major Tom is asked to open by the rest of the system opens the same way a location typed into the address field does: a `gemini://` link activated in another application, and a Gemtext or plain-text document opened from the Finder, each become a new selected tab of the frontmost window, or the first window when none is open. A launch caused by one of these shows it alongside any restored session rather than behind the homepage. Opening a location never creates a window the reader cannot see.
...
Preformatted blocks may carry ANSI SGR escape sequences, which some capsules use to color ASCII art. Major Tom renders their colors and font attributes and nothing else: it is not a terminal emulator, so cursor, erase, and report sequences are discarded, and attributes reset at every line ending because authors set them afresh per line. Escapes are honored only inside preformatted blocks; elsewhere in a gemtext document they stay as sent. A capsule's foreground color is adjusted—only as far as it must be, and without changing its hue—to stay readable on the reader's content theme. A background color is honored only where the capsule also set a foreground, because a background on its own can hide text the capsule never chose the color of; background rendering is off by default. Escape sequences never reach the displayed text, on or off: they are control codes, not characters an author meant a reader to see.
