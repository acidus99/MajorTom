...
The manager imports combined, unencrypted PEM client identities containing an X.509 certificate and RSA private key in either order. Users can import an identity from the clipboard, including with Command-V, or choose up to two files with the native file picker. A single selected file must contain the combined identity. When two files are selected, one must contain only the certificate and the other only the private key; other two-file combinations are rejected. Before enabling **Import**, Major Tom parses PKCS #1 or PKCS #8 RSA private keys, proves that the private key matches the certificate's public key, and verifies that the stored identity can perform the signature required by TLS. It then displays the certificate with the standard macOS inspector and stores the identity using the same Keychain policy as a locally created identity. When the source was the clipboard, the sheet offers to clear the copied private key after a successful import. Reimporting an existing identity validates or repairs its Keychain items in place so its identifier and capsule approvals remain intact. Encrypted, unsupported, incomplete, and mismatched identities produce specific errors.
